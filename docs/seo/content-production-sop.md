...
- Before scoring the next page, run `npm run growth:collect` to record every published page in one atomic 28-complete-Shanghai-day portfolio. Include it through `portfolioSnapshot` or `portfolioFunnels`; do not hand-pick only the best-performing page. A failed credential or endpoint remains an unavailable page entry, and currency boundaries and orphan callbacks remain visible.
...
- Four existing pages are the present cold-start allowance. Once that allowance is reached, at least one page must have observed landing UV before another new page can pass the builder. An update additionally needs an observed Search Console row for its exact target URL.
