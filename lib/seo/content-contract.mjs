import { markdownSemanticBlockCount, parseMarkdownBlocks } from "./markdown-semantics.mjs";
...
const STORY_DRIVEN_ADVENTURE_RENDERER = "story_driven_adventure";
const STORY_DRIVEN_SCENARIO_FIELDS = Object.freeze([
  "role",
  "situation",
  "pressure",
  "decision",
...
const STORY_DRIVEN_FIELD_PATTERN = /^\*\*([^*\n]+):\*\*\s+([\s\S]+)$/;

export function validateRendererSpecificContent({ architecture, signatureModule }) {
  if (architecture?.presentation?.rendererId !== STORY_DRIVEN_ADVENTURE_RENDERER) return true;

  assert(architecture?.content?.signature?.type === "scenario" && signatureModule?.type === "scenario",
    "Story-driven adventure renderer requires a scenario signature module");
  assert(Array.isArray(signatureModule.items) && signatureModule.items.length === 3,
    "Story-driven adventure renderer requires exactly three scenario signature items");

  for (let index = 0; index < signatureModule.items.length; index += 1) {
    const item = signatureModule.items[index];
    const blocks = parseMarkdownBlocks(item?.bodyMarkdown);
    const fields = blocks.map((block) => block.type === "prose"
      ? block.text.match(STORY_DRIVEN_FIELD_PATTERN)
      : null);
    assert(fields.length === STORY_DRIVEN_SCENARIO_FIELDS.length && fields.every(Boolean),
      `Story-driven scenario item ${index + 1} must contain exactly six labelled prose fields`);
    const labels = fields.map((match) => match[1].trim().toLowerCase());
    assert(labels.every((label, fieldIndex) => label === STORY_DRIVEN_SCENARIO_FIELDS[fieldIndex]),
      `Story-driven scenario item ${index + 1} must use Role, Situation, Pressure, Decision, First line, and Voice direction in order`);
    assert(fields.every((match) => isString(match[2], 3)),
      `Story-driven scenario item ${index + 1} has an empty labelled field`);
  }
  return true;
}
...
export function validatePublishedPageArchitecture({ page, architecturePolicy, presentationCatalog }) {
  ...
  validateRendererSpecificContent({ architecture, signatureModule: signature });
