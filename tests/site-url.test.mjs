...
test("site configuration declares the Playworlds LoreLens canonical and legacy origins", () => {
  assert.deepEqual(siteConfig, {
    schemaVersion: 1,
    canonicalOrigin: "https://lorelens.playworlds.ai",
    legacyOrigins: [
...
test("getSiteUrl defaults to the configured canonical root origin", () => {
  withSiteUrl(undefined, () => {
    ...
    assert.equal(site.toString(), "https://lorelens.playworlds.ai/");
    assert.equal(site.origin, "https://lorelens.playworlds.ai");
...
test("getSiteUrl normalizes host casing and the root trailing slash", () => {
  withSiteUrl("  HTTPS://LORELENS.PLAYWORLDS.AI  ", () => {
    assert.equal(getSiteUrl().toString(), "https://lorelens.playworlds.ai/");
    assert.equal(
      absoluteSiteUrl("/sitemap.xml"),
      "https://lorelens.playworlds.ai/sitemap.xml",
    );
  });
});
...
test("getSiteUrl rejects non-root, credentialed, and non-HTTPS public URLs", () => {
  const invalidValues = [
    ...
    "ftp://lorelens.playworlds.ai/",
    "https://reader:secret@lorelens.playworlds.ai/",
    "https://lorelens.playworlds.ai/guides",
